...
async function bridge(method, ...args) {
  if (typeof window.pywebview?.api?.[method] !== 'function') {
    throw new Error(`管理端桥接尚未准备好（${method}）`);
  }
...
async function login(event) {
  ...
  const button = event.submitter || $('#login-button');
  const error = $('#login-error');
  error.textContent = '';
  if (!state.bridgeReady || !window.pywebview?.api) {
    error.textContent = '管理端仍在初始化，请稍后重试';
    return;
  }
  setBusy(button, true);
  try {
    ...
    error.textContent = exception?.message || String(exception) || '登录失败';
  } finally {
...
async function initializeBridge() {
  if (state.bridgeReady || state.bridgeInitializing || !bridgeMethodsReady()) return false;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const bridgePoller = setInterval(async () => {
    ...
    if (await initializeBridge()) {
      ...
      clearInterval(bridgePoller);
      const loginButton = $('#login-button');
      loginButton.textContent = '初始化失败';
      $('#login-error').textContent = '管理端桥接初始化失败，请重新打开程序';
    }
  }, 50);
});
